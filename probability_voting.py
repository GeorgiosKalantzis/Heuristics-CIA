...
for i in range(len(users)):
    ...
    prob=np.array([[0.0 for x in range(len(y_test))]for y in range(len(models))])
    ...
    for w in range(len(y_test)):
            ...
            for z in range(len(models)):
                ...
                #Due to the property of the logistic function 1-f(x)=f(-x)
                #we obtain the probability that the sample is that of the user
                #regardless of sgn(d[z,w])
                prob[z,w]=((1/(1+exp(-(d[z,w])))))

            probabilities_sum=(sum(prob[:,w])/len(models))
            if probabilities_sum>=0.5:
